...
import { Trash2, Check, Upload, Globe } from "lucide-react";

const SlotRow = memo(({ slot, hasTexture, onUpload, onApplyToAll, onClear }) => (
    <div
    ...
    >
        ...
        <div className="flex gap-1.5">
            <label
                ...
                title={hasTexture ? "Reload texture" : "Upload to this mesh only"}
            >
            ...
            <label
                ...
                title="Upload and apply to ALL meshes"
            >
                <Globe size={10} />
                To All
                <input
                    ...
                    onChange={(e) => {
                        const files = Array.from(e.target.files ?? []);
                        if (files.length > 0) onApplyToAll(files);
                        e.target.value = "";
                    }}
                />
            </label>
