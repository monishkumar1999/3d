...
import { ChevronDown, Scaling, Droplet, MoreHorizontal, Copy, BringToFront, SendToBack, Trash, Pencil } from 'lucide-react';
...
const FloatingTextToolbar = ({
    ...
    const [editTextValue, setEditTextValue] = useState(sticker?.text || '');
    ...
    useEffect(() => {
        if (sticker?.text) setEditTextValue(sticker.text);
    }, [sticker?.text]);
                ...
                <div className="relative">
                    <button
                        onClick={(e) => {
                            ...
                            setShowFonts(false); setShowColors(false); setShowMore(false); setShowOpacity(false);
                        }}
                    ...
                    {showEditText && (
                        <div className="absolute top-full left-0 mt-2 w-64 bg-[#1e1e1e] border border-zinc-700/50 rounded-xl shadow-xl p-3 z-[200]">
                            ...
                            <div className="flex gap-2 mt-2">
                                <button onClick={() => setShowEditText(false)} className="flex-1 py-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-lg text-[10px] font-bold transition-all">Cancel</button>
                                <button onClick={() => { if (editTextValue.trim()) { updateSticker('text', editTextValue.trim()); setShowEditText(false); } }} disabled={!editTextValue.trim()} className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:bg-zinc-700 disabled:text-zinc-500 text-white rounded-lg text-[10px] font-bold transition-all">Save</button>
                            </div>
                        </div>
                    )}
                </div>
                ...
                <div className="relative">
                    <button
                        onClick={(e) => {
                            ...
                            setShowColors(false); setShowMore(false); setShowEditText(false); setShowOpacity(false);
                        }}
                    ...
                    {showFonts && <FontSelector activeFamily={sticker.fontFamily} onSelect={(fam) => { updateSticker('fontFamily', fam); setShowFonts(false); }} />}
                </div>
                ...
                <div className="relative">
                    <button
                        onClick={(e) => {
                            ...
                            setShowFonts(false); setShowMore(false); setShowEditText(false); setShowOpacity(false);
                        }}
                        className="w-8 h-8 rounded-full flex items-center justify-center hover:bg-zinc-800 transition-colors"
                        title="Text Color"
                    >
                        <div className="w-4 h-4 rounded-full border border-white/20 shadow-sm" style={{ backgroundColor: sticker.fill }} />
                    </button>
                    {showColors && <ColorPicker currentColor={sticker.fill} onSelectColor={(col) => updateSticker('fill', col)} />}
                </div>
                ...
                <div className="relative">
                    <button
                        onClick={(e) => {
                            ...
                            setShowFonts(false); setShowColors(false); setShowMore(false); setShowEditText(false);
                        }}
                ...
                <div className="relative">
                    <button
                        onClick={(e) => {
                            ...
                            setShowFonts(false); setShowColors(false); setShowEditText(false); setShowOpacity(false);
                        }}
                    ...
                    {showMore && (
                        <div className="absolute top-full right-0 mt-2 w-48 bg-[#1e1e1e] border border-zinc-700/50 rounded-xl shadow-xl p-1 z-[200] flex flex-col gap-0.5">
                            <button onClick={() => { onDuplicate(); setShowMore(false); }} className="flex items-center gap-2 px-3 py-2 text-xs font-medium text-zinc-300 hover:bg-zinc-800 rounded-lg text-left"><Copy size={13} /> Duplicate</button>
                            <button onClick={() => { onMoveForward(); setShowMore(false); }} className="flex items-center gap-2 px-3 py-2 text-xs font-medium text-zinc-300 hover:bg-zinc-800 rounded-lg text-left"><BringToFront size={13} /> Bring Forward</button>
                            <button onClick={() => { onMoveBackward(); setShowMore(false); }} className="flex items-center gap-2 px-3 py-2 text-xs font-medium text-zinc-300 hover:bg-zinc-800 rounded-lg text-left"><SendToBack size={13} /> Send Backward</button>
                            <div className="h-px bg-zinc-700 my-1" />
                            <button onClick={() => { onDelete(); setShowMore(false); }} className="flex items-center gap-2 px-3 py-2 text-xs font-medium text-red-400 hover:bg-red-400/10 rounded-lg text-left"><Trash size={13} /> Delete</button>
                        </div>
                    )}
                </div>
            </div>
