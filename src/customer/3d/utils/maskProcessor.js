import { getCachedImageFast, setCachedImageFast } from './imageCache';
 ...
 * Results are cached in localStorage for faster subsequent loads.
 * 
...
export const processWireframeToSolid = (imageUrl) => {
    return new Promise((resolve, reject) => {
        // Check cache first
        const cached = getCachedImageFast(imageUrl);
        if (cached) {
            resolve(cached);
            return;
        }
        ...
        img.onload = () => {
            ...
            setCachedImageFast(imageUrl, resultDataUrl);
